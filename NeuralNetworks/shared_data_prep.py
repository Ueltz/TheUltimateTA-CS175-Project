"""
This file is used to normalize and denormalize data from asap 1 and 2, as well as initially load and split the data.

Written By: Navid Riahi
...
Usage:
    ...
    data = load_all_data(
        asap1_path='training_set_rel3.csv',
        asap2_path='ASAP2_train_sourcetexts.csv',
    )
...
def load_asap1(path: str) -> pd.DataFrame:
    ...
    df = pd.read_csv(path)
    #df = pd.read_csv(path).fillna(0.5)
...
def load_asap2(path: str) -> pd.DataFrame:
    ...
    result['economically_disadvantaged'] = df['economically_disadvantaged']
    result['student_disability_status'] = df['student_disability_status']
    result['ell_status'] = df['ell_status']
    result['race_ethnicity'] = df['race_ethnicity']
...
def load_all_data(
    asap1_path: str = 'training_set_rel3.csv',
    asap2_path: str = 'ASAP2_train_sourcetexts.csv',
    val_fraction: float = DEFAULT_VAL_FRACTION,
    seed: int = DEFAULT_SEED,
) -> Dict:
    """
    ...
    print(f"\n{'=' * 60}")
    print("Columns in asap1 DataFrame:")
    print("  essay_id, prompt_id, text, raw_score, norm_score")
    print(f"{'=' * 60}")

    print(f"\n{'=' * 60}")
    print("Columns in asap2 DataFrame:")
    print("  essay_id, prompt_id, economically_disadvantaged, student_disability_status, ell_status, race_ethnicity, gender, text, raw_score, norm_score")
    print(f"{'=' * 60}")
